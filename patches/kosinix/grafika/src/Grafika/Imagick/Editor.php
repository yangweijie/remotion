 ...
 * 
 * 补丁版本：修复 Imagick 兼容性问题
 * - opacity(): 使用 evaluateImage 替代已弃用的 setImageOpacity
 * - blend(): 支持带偏移的裁剪合并
 * 
...
final class Editor implements EditorInterface
{
     ...
     *
     * 补丁修复：使用 evaluateImage 替代已弃用的 setImageOpacity
     * 
     * @param Image $image
     * @param float $opacity
     *
    ...
    public function opacity(&$image, $opacity)
    {
        ...
        // 补丁修复：使用 evaluateImage 替代已弃用的 setImageOpacity
        // setImageOpacity 在新版 Imagick 中已被移除
        // 使用 setImagickOpacity 方法，如果不存在则使用 evaluateImage
        $core = $image->getCore();
        
        // 方法1：尝试使用 setImagickOpacity (Imagick 6.3.6+)
        if (method_exists($core, 'setImagickOpacity')) {
            $core->setImagickOpacity($opacity);
        } else {
            // 方法2：使用 evaluateImage 修改 alpha 通道
            // 将透明度从 0-1 映射到 0-QuantumRange
            $quantumRange = $core->getQuantumRange();
            $quantum = $quantumRange['quantumRangeLong'];
            $opacityValue = $opacity * $quantum;
            
            // 首先确保图像有 alpha 通道
            $core->setImageAlphaChannel(\Imagick::ALPHACHANNEL_ACTIVATE);
            
            // 使用乘法操作调整透明度
            // Multiply 当前 alpha 值与目标 opacity
            $core->evaluateImage(\Imagick::EVALUATE_MULTIPLY, $opacity, \Imagick::CHANNEL_ALPHA);
        }
